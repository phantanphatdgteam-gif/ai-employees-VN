 ...
 * strategy file, or a dashboard partial.
 *
 ...
 *                   folder, then the CSAT_ROOT environment variable. Used to
 *                   find strategy/tone.md and strategy/proof-inventory.md.
 ...
 *      that exact string appears verbatim in strategy/proof-inventory.md.
 ...
 *   5. A hashtag, where tone.md sets the hashtag policy to none.
 ...
 *
 *   strategy skips the tone rules because strategy/tone.md is the file that
 *   holds the banned lists, and a checker that fails its own vocabulary file
 *   is useless. It is also why strategy/themes.md passes with a match phrase
 *   in it that would fail anywhere else. dashboard skips hashtag and autolink
...
const COUNT_NOUNS = [
  ...
  // This Employee's own vocabulary. Every one of these is a number a support
  // desk states casually and cannot defend, and the ones that move money are
  // the ones a member would be held to.
  "tickets?", "complaints?", "cases?", "conversations?", "threads?",
  "messages?", "exchanges?", "escalations?", "issues?", "bugs?",
  "incidents?", "outages?", "reviews?", "ratings?", "stars?",
  "refunds?", "credits?", "cancellations?", "chargebacks?", "renewals?",
  "saves?", "churns?", "complaints?", "drafts?",
];
...
// A bracketed source. A block that carries one has named where its numbers
// came from, so the metrics inside that block are sourced and pass.
//
// The scope is the block, not the line, because a dossier states four claims
// and closes with one bracket underneath them all. A block is what sits
// between two blank lines.
//
// Three shapes count as a source: a path with a slash in it, a bare kit
// filename with a data extension, or a screen named with the date it was read
// on. Nothing else does, which is why [redacted: api-key] and
// [member: confirm this is granted before you send] are markers rather than
// sources and suppress nothing.
const SOURCE_BRACKET = new RegExp(
  "\\[[^\\]\\n]*(?:" +
    "[A-Za-z0-9_-]+\\/[A-Za-z0-9._*\\/-]+" + "|" +
    "\\b[A-Za-z0-9_-]+\\.(?:md|jsonl|json|csv|log|txt|html)\\b" + "|" +
    "\\b\\d{4}-\\d{2}-\\d{2}\\b" +
  ")[^\\]\\n]*\\]",
  "g"
);
...
/* ------------------------------------------------------------------ *
 * Rule 4. Voice. strategy/tone.md overrides these lists completely when it
...
function loadVoice(root) {
  const file = path.join(root, "strategy", "tone.md");
  ...
  // tone.md overrides completely where it has content, and falls back per
...
function loadProof(root) {
  ...
  return { allowed: allowed.join("\n"), structural, source: "strategy/proof-inventory.md" };
}
...
function check(text, dest, ctx) {
  ...
  /* Rule 2. Metrics. */
  const metricView = maskRanges(
    prose,
    rangesOf(prose, NOT_A_METRIC).concat(sourcedBlockRanges(prose))
  );
  for (const [kind, pattern] of METRIC_PATTERNS) {
    ...
    while ((m = re.exec(metricView)) !== null) {
      ...
      at(m.index, m[0].length, "metric",
        ...
        (ctx.proof.source === "missing" ? "strategy/proof-inventory.md, which is missing" : "strategy/proof-inventory.md"));
    }
  }
  ...
  for (const bad of ctx.proof.structural) {
    v.push({
      ...
      detail: "strategy/proof-inventory.md, under ## Agent sourced, has an entry that is not " +
...
/*
 * Every block that carries a bracketed source, as offset ranges. A block runs
 * from one blank line to the next, so a bracket on its own line still covers
 * the claims above it, which is exactly how a dossier is written.
 */
function sourcedBlockRanges(text) {
  const ranges = [];
  let blockStart = 0;
  let cursor = 0;
  ...
  const flush = (end) => {
    ...
    SOURCE_BRACKET.lastIndex = 0;
    if (SOURCE_BRACKET.test(block)) ranges.push([blockStart, end]);
  };
...
function selftest() {
  const ctx = { voice: { ...FALLBACK_VOICE, source: "fallback" }, proof: { allowed: "", structural: [], source: "missing" } };
  const withProof = { voice: ctx.voice, proof: { allowed: "42% reply rate", structural: [], source: "strategy/proof-inventory.md" } };

  const cases = [
    ["clean reply copy", "Hi Sam,\n\nYou were charged twice on the second. I can see both payments on the account.\n\nI have passed it to the person who can put it right today.\n\nThanks for writing in.\n", "email", ctx, []],
    ...
    ["unsourced count", "We answer 40 tickets a week.", "email", ctx, ["metric"]],
    ["an ISO date is not a metric", "Observed on 2026-03-04 and replied 2026-03-05.", "plain", ctx, []],
    ["a card id is not a metric", "D-014 is ready and D-018 waits on you.", "plain", ctx, []],
    ["a redaction marker survives", "They pasted [redacted: api-key] into the ticket.", "plain", ctx, []],
    ["a member marker survives", "[member: confirm this is granted before you send]", "email", ctx, []],
    ...
    ["strategy skips the tone rules", "Banned words: supercharge, unlock, revolutionary.", "strategy", ctx, []],
    ["an unsourced ticket count", "4 tickets in the last month.", "plain", ctx, ["metric"]],
    ["a bracketed ledger path sources it", "4 tickets since 2026-02-08 [tickets/tickets.jsonl]", "plain", ctx, []],
    ["a bracket covers the whole block", "- repeat_contact: 4 tickets since 2026-02-08\n  and 2 of them ended in a refund\n  [tickets/tickets.jsonl]\n", "plain", ctx, []],
    ["a bracket does not reach the next block", "- repeat_contact: 4 tickets\n  [tickets/tickets.jsonl]\n\nAnother 7 customers wrote in.\n", "plain", ctx, ["metric"]],
    ["a redaction marker is not a source", "They sent 3 refunds [redacted: api-key]", "plain", ctx, ["metric"]],
    ["a member marker is not a source", "Issue 2 credits [member: confirm this is granted before you send]", "plain", ctx, ["metric"]],
  ];
...
function resolveRoot(opts) {
  ...
  if (process.env.CSAT_ROOT) return path.resolve(process.env.CSAT_ROOT);
...
function main() {
  ...
  if (!DESTINATIONS.includes(opts.dest)) {
    const guess = { "linkedin-dm": "dm", "linkedin": "dm", "file": "strategy", "internal": "strategy", "html": "dashboard", "text": "plain", "queue": "email" }[opts.dest];
  ...
  const ctx = { voice: loadVoice(root), proof: loadProof(root) };
