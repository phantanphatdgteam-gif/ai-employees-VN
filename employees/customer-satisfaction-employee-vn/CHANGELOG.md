# Customer Satisfaction Employee: release history
...
- `csat-desk-intake` writes `strategy/proof-inventory.md` exactly as `CONTRACT.md` shows it, and that template carries `Format: <the exact string that may appear in copy> | <ledger path it was read from> | <YYYY-MM-DD>` under `## Agent sourced`. The script read that line as an agent sourced entry without a real date, so every copy check in the install failed with a `proof-inventory` violation whatever the text said. A line that starts with `Format:` and still holds a `<...>` placeholder is now skipped as documentation. A real entry without a ledger path or a date is still rejected.
...
- The same fix as Customer Satisfaction Employee 1.8.1. `scripts/` stays byte identical to the original kit.

## 1.9.0, 2026-09-24

The Vietnam localization pass: `customer-satisfaction-employee-vn` is a localized variant of `customer-satisfaction-employee` 1.8.0, built from the Vietnamese expert form for the customer care role (Phần A and sections B-1 to B-8). The form was filled by an AI and no clause has yet been confirmed by a Vietnamese practitioner.

- All eight routines were localized from their own form section: `csat-desk-intake`, `csat-inbox-sweep`, `csat-reply-desk`, `csat-churn-watch`, `csat-deflection-desk`, `csat-taxonomy-refresh`, `csat-satisfaction-report` and `csat-desk-standup`. Routine instructions stay in English; every line you read is Vietnamese, and every heading, id, status, key and `n/a` token another file parses is unchanged.
- The desk now works where Vietnamese customers write: marketplace seller chat and return requests, social page inboxes, official chat accounts, comments, calls, and map reviews, each mapped onto the five existing channel values. Marketplace chat and return requests are read first on every sweep, and ticket lines can carry the time the customer wrote, the platform's own deadline, the time of the shop's first reply, and what kind of input the line holds.
- Platform reply windows, return deadlines and the legal time to acknowledge a complaint are read from your own files with their source and date, never from a routine. The standup lists every deadline that ends before the next brief, and a `Tết` day or a vacation mode period is handled from what you recorded.
- Replies and macros follow Vietnamese practice: `Dạ`, `em` and `anh/chị`; nothing that leads a marketplace customer off the marketplace; never a request for a one time code or a fee; nothing traded for a rating; no promotion without consent; addresses in the current ward or commune and province; an invoice correction and a request about a customer's own data are yours, never a routine's.
- `CONTRACT.md` section 10, new, "Vietnam operating rules": the twelve rules three or more routines share, the lines this variant adds under the existing strategy headings with the `tạm` fallbacks and the legal sources the intake transcribes from, the four new ticket fields, the card `owner` role, the one account join by order code, and the automated assistant sentence on a released send. Section 8.4 now reads this variant's own published version and names the fork's installer for an upgrade.
- `CAPABILITIES.md` section 4c, new: the platform terms table opened on 24/09/2026 (Shopee, Facebook and Instagram, TikTok Shop, Lazada, Zalo, Google Maps). No row permits a browser: your own account on those platforms is read through a connector you authorised or an export you drop into `exports/`. New read only routes `marketplace.read`, `calls.read` and `replytool.saved`, and the Vietnamese routes for `billing.read` and `reviews.read`, all at `unknown` until you confirm them.
- `SCHEDULE.md`: no row moved. Section 9 says why the weekday rows stay `mon-fri` and how to add Saturday cover. `ROLE.md`, `README.md`, `INSTALL-PROMPT.md` and `AGENTS.md` name the Vietnamese fields the install now asks for and say that this variant is installed and upgraded only from the Vietnam fork, never from the public npm package.
- Scripts are byte identical to 1.8.0.
...
- `routines/csat-desk-standup/SKILL.md`, the step that writes the brief, gains the delivery line.
...
- `csat-desk-intake`, Step B3a, new: once a month it reads the published `VERSION` of this kit, and where there is a newer one it writes what you get, in at most five plain lines, to `state/kit-update.json`. `csat-desk-standup` carries it in the next brief under `## About this kit`, in full once per version and as a short reminder once a month after that, with the two lines that take the update. No routine runs the upgrade, and no routine runs `npx`.
...
- `csat-inbox-sweep` resolves each channel through 4b before it opens a tab. No routine names a vendor; the routes live in section 4b and nowhere else.
