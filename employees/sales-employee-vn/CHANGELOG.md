...
- The same fix as Sales Employee 1.8.1. `scripts/` stays byte identical to the original kit.

## 1.9.0, 2026-09-24

The Vietnam variant, `sales-employee-vn`: a localized version of the Sales Employee 1.8.0, built from the Vietnamese expert form for this role (07, Sales) and its dated source index, with the routines kept in English and every line the member reads written in Vietnamese. The form was filled by an AI and reviewed by the project owner; no Vietnamese sales practitioner has confirmed a rule yet.

- All seven routines localized, each from its own form section and a clause ledger kept outside the kit. What they follow in common is written once in `CONTRACT.md` section 10, "Vietnam operating rules": platform terms decide what is read, no advertising contact without a consent the member records, a refusal on any channel ends contact, no bought or unsourced list, a captcha is never cleared, a superlative needs its document, no price, tax rate or discount the member did not set, no figure from another market, calls and chat messages are always a person's, and no legal threshold in any kit file.
- `sales-first-touch-drafts` drafts a first touch only for a person whose row above the marker in `crm/contacts.csv` carries a `consent-email-YYYY-MM-DD` or `consent-linkedin-YYYY-MM-DD` tag, and every advertising email, first touch and follow up, carries the `[QC]` subject label, the sender's details and a refusal line. `sales-desk-standup` names the people held for want of a consent record, and puts a stop line first for anyone who refused while a draft still waits.
- `sales-prospect-sweep` no longer opens LinkedIn or any platform whose terms forbid automated reading, reads the registration portal and the tax status lookup one company at a time, rests a dead source instead of retiring it, and refuses bought lists. `CAPABILITIES.md` gains `## Platform terms, Vietnam`, the Vietnamese CRM and call log routes at `unknown`, and the held outbound actions.
- `sales-desk-setup` writes `## Legal identity`, the member's own price lines, `## Form of address`, the signature block and `influencer:`, writes the operating status test as `supporting`, seeds `review/manual.md` with `rate_floor: 15`, and checks for a newer version of this variant, never of `sales-employee`. `CONTRACT.md` 8.4 prints the fork installer's upgrade line.
- `sales-pipeline-review` and `sales-qualification-refresh` use evidence floors sized for a small business with one or two people calling. The refresh makes no change on a month shortened by declared holidays or missing weekly reviews, and the review changes a follow up framework's angle, never the cadence.
- No schedule row, window or budget changed. `examples/` now shows one fictional Vietnamese day; `examples/queue/2026-09-22-first-touch.md` replaces the English queue example.
...
- `routines/sales-desk-standup/SKILL.md`, the step that writes the brief, gains the delivery line.
...
- `sales-desk-setup`, Step B3a, new: once a month it reads the published `VERSION` of this kit, and where there is a newer one it writes what you get, in at most five plain lines, to `state/kit-update.json`. `sales-desk-standup` carries it in the next brief under `## About this kit`, in full once per version and as a short reminder once a month after that, with the two lines that take the update. No routine runs the upgrade, and no routine runs `npx`.
...
- `sales-followup-sweep` resolves the reply read through 4b before it takes the lane. No routine names a vendor; the routes live in section 4b and nowhere else.
