...
**Sales Employee**, one of the eight AI Employees from [github.com/markfulton/ai-employees](https://github.com/markfulton/ai-employees).
...
## This is the Vietnam variant

This folder is `sales-employee-vn`. The routines are in English and the member reads Vietnamese. The Vietnam rules every routine follows are written once in `CONTRACT.md` section 10, and the platforms whose terms forbid automated reading are listed under `## Platform terms, Vietnam` at the end of `CAPABILITIES.md`. Never upgrade this folder from the public npm package or onto `sales-employee`: that would replace the Vietnamese routines. The fork's bundled installer upgrades it as `sales-employee-vn`.

## If your harness has an Instructions field instead of a folder

On a hosted agent such as Grok Bot, the kit lives on the bot's own cloud computer and the bot reads an Instructions field before every task. Paste this whole file into that field, with the folder's absolute path on that computer in place of `«SALES_ROOT»`. It is the base layer (the two guardrails and the files that outrank everything else), the role layer (`ROLE.md`, which names the evidence standard) and the current focus (the strategy folder the routines keep current), and it is short enough to fit. Everything else stays in the files.

## If you have been asked to install this Employee

The person opened you in this folder and said something like "install the Sales Employee from this folder". Read `INSTALL-PROMPT.md` and follow everything between `=== BEGIN PROMPT ===` and `=== END PROMPT ===` exactly as if they had pasted it. `«SALES_ROOT»` is this folder, resolved to an absolute path. Where the `FILL THIS IN` block is unedited, work the lines out yourself the way the prompt says, and ask one question only if you cannot.
...
|---|---|
| `sales-desk-setup` | `routines/sales-desk-setup/SKILL.md` |
| `sales-desk-standup` | `routines/sales-desk-standup/SKILL.md` |
| `sales-first-touch-drafts` | `routines/sales-first-touch-drafts/SKILL.md` |
| `sales-followup-sweep` | `routines/sales-followup-sweep/SKILL.md` |
| `sales-pipeline-review` | `routines/sales-pipeline-review/SKILL.md` |
| `sales-prospect-sweep` | `routines/sales-prospect-sweep/SKILL.md` |
| `sales-qualification-refresh` | `routines/sales-qualification-refresh/SKILL.md` |
