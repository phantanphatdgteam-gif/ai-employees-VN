...
/*
 * runlog.mjs
 * The only sanctioned way to append a run record to «SALES_ROOT»/runlog.jsonl.
 *
 ...
 *     node scripts/runlog.mjs --routine sales-first-touch-drafts --period 2026-03-04 \
 *          --start <ISO> --end <ISO> --status ok \
 *          --output "queue/2026-03-04-first-touch.md (4 entries)" \
 ...
 *                   folder, then the SALES_ROOT environment variable.
 *   --log <path>    Write to this file instead of «SALES_ROOT»/runlog.jsonl.
 ...
 *    "status":"ok",
 *    "outputs":["queue/2026-03-04-first-touch.md (4 entries)"],
 *    "blockers":[],
 *    "notes":"drafts count verified; follow up interval 4 days"}
 *
 * EXIT CODES
...
const CADENCE = {
  "sales-prospect-sweep": "day",
  "sales-desk-standup": "day",
  "sales-first-touch-drafts": "day",
  "sales-followup-sweep": "day",
  "sales-pipeline-review": "week",
  "sales-desk-setup": "month",
  "sales-qualification-refresh": "month",
};

// CONTRACT.md Appendix A. This kit has never shipped under another naming
// scheme, so there are no stale ids to migrate. What a member does get wrong
// by hand is the prefix, and a prefix-less id is a run record nobody can
// group, so name the correct id here rather than letting it through.
const STALE_IDS = {
  "prospect-sweep": "sales-prospect-sweep",
  "desk-standup": "sales-desk-standup",
  "standup": "sales-desk-standup",
  "first-touch-drafts": "sales-first-touch-drafts",
  "first-touch": "sales-first-touch-drafts",
  "followup-sweep": "sales-followup-sweep",
  "follow-up-sweep": "sales-followup-sweep",
  "pipeline-review": "sales-pipeline-review",
  "desk-setup": "sales-desk-setup",
  "setup": "sales-desk-setup",
  "qualification-refresh": "sales-qualification-refresh",
};
...
function resolveRoot(opts) {
  ...
  if (process.env.SALES_ROOT) return path.resolve(process.env.SALES_ROOT);
  // The script lives at «SALES_ROOT»/scripts/runlog.mjs, so its parent is the
...
function validateShape(record) {
  ...
  } else if (!CADENCE[routine] && !routine.startsWith("sales-")) {
    problems.push("routine \"" + routine + "\" is not one of the seven and does not carry the sales- prefix");
  }
...
function good() {
  return {
    routine: "sales-first-touch-drafts",
    period: "2026-03-04",
    start: "2026-03-04T08:15:11+07:00",
    end: "2026-03-04T08:44:40+07:00",
    status: "ok",
    outputs: ["queue/2026-03-04-first-touch.md (4 entries)", "crm/contacted.jsonl (+4 queued)"],
    blockers: [],
    notes: "1 draft dropped by copy-check, unsourced number; drafts count verified",
  };
}
...
function selftest() {
  ...
  add("weekly key on a weekly routine", (r) => { r.routine = "sales-pipeline-review"; r.period = "2026-W10"; }, false, false);
  add("monthly key on a monthly routine", (r) => { r.routine = "sales-qualification-refresh"; r.period = "2026-03"; }, false, false);
  add("the paused status", (r) => { r.status = "skipped-paused"; }, false, false);
  ...
  add("routine id written without its prefix", (r) => { r.routine = "first-touch-drafts"; }, true, false);
  ...
  try {
    ...
    fs.writeFileSync(tmpLog, "\uFEFF{\"routine\":\"sales-desk-standup\",\"period\":\"2026-03-03\"}");
