...
**This is the Vietnam variant, `social-media-employee-vn`.** Routines are written in English; every line the member reads is Vietnamese, and every heading, id and status another file parses stays in English. `CONTRACT.md` section 10 holds the Vietnam operating rules, and it outranks a routine that seems to read otherwise. No routine drives a personal chat app: the person on duty pastes and sends. The public npm package does not contain this variant, so never upgrade it with `npx ai-employees upgrade social-media-employee`.
...
|---|---|
| `soc-calendar-standup` | `routines/soc-calendar-standup/SKILL.md` |
| `soc-draft-queue` | `routines/soc-draft-queue/SKILL.md` |
| `soc-engagement-sweep` | `routines/soc-engagement-sweep/SKILL.md` |
| `soc-intake-and-voice` | `routines/soc-intake-and-voice/SKILL.md` |
| `soc-material-sweep` | `routines/soc-material-sweep/SKILL.md` |
| `soc-performance-review` | `routines/soc-performance-review/SKILL.md` |
| `soc-publish-run` | `routines/soc-publish-run/SKILL.md` |
