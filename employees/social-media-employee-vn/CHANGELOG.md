# Social Media Employee: release history
...
- `soc-intake-and-voice` writes `voice/proof-inventory.md` exactly as `CONTRACT.md` shows it, and that template carries `Format: <the exact string that may appear in copy> | <ledger path it was read from> | <YYYY-MM-DD>` under `## Agent sourced`. The script read that line as an agent sourced entry without a real date, so every copy check in the install failed with a `proof-inventory` violation whatever the text said. A line that starts with `Format:` and still holds a `<...>` placeholder is now skipped as documentation. A real entry without a ledger path or a date is still rejected.
...
- The same fix as Social Media Employee 1.8.2. `scripts/` stays byte identical to the original kit.

## 1.9.0, 2026-09-24

The Vietnam variant, `social-media-employee-vn`, localized from `social-media-employee` 1.8.1 with the Vietnamese expert form for the Social Media role (phiếu 06). The form was filled by an AI and no clause has yet been confirmed by a Vietnamese practitioner.

- All seven routines localized, one form section each (B-1 to B-7): instructions stay in English, every line the member reads is Vietnamese, and every heading, key, id and status another file reads stays in English. Step 0, both guardrails, the run record, idempotency and every `## Corrections` section are unchanged.
- `CONTRACT.md` section 10, new, "Vietnam operating rules": the eleven rules three or more routines share, what this variant adds to the plan files, two ledger rules (the `[đã che]` mask in `engagement/inbound.jsonl` and the new `Lượt xem` series), and the brief in Vietnamese. Every existing section number is unchanged, and no legal figure is written into the kit.
- Schedule: the standup fires at 07:10 so the brief is ready when the owner reads it, the publish run at 08:40 after that reading, the material sweep at 09:20 and the draft queue at 10:15 to keep the browser spacing, and the Friday review at 15:50 so the page is ready by 16:30. `SCHEDULE.md` section 4.2 carries a new `veto_lead` line the standup reads to name an early post a day ahead.
- `CAPABILITIES.md`: platform terms read on 24/09/2026 and a platform notes table (terms verdict, hashtag cap, opening cadence, minimum spacing, starting caption length); per destination scheduling notes; Zalo OA as a connected route only, read only and at `unknown`; own account analytics routes; no brief delivery into Zalo.
- The monthly kit version check reads this variant's own folder, and the upgrade line uses the fork's bundled installer, so an upgrade never replaces the Vietnamese routines with the English ones.
- `INSTALL-PROMPT.md` asks for the form of address, working days and closed days, restricted categories and certificates, influencer bookings, the current address, and the folders for the daily drop, price list and season file. `README.md`, `AGENTS.md` and the examples follow.
- After an independent review of every routine: `CONTRACT.md` 10.1 lets a routine write a stable blocker in Vietnamese while fixed blockers stay in English; 10.2 says the 24/11 day off line is a closed day only until the member says they open; the 2.6 queue header matches the draft queue; the marketplace row in `CAPABILITIES.md` records that Shopee also bars tracking another shop by hand; `examples/calendar/CALENDAR.md` shows the Vietnamese calendar the standup renders.
...
- `routines/soc-calendar-standup/SKILL.md`, the step that writes the brief, gains the delivery line.
...
- `soc-intake-and-voice`, Step B3a, new: once a month it reads the published `VERSION` of this kit, and where there is a newer one it writes what you get, in at most five plain lines, to `state/kit-update.json`. `soc-calendar-standup` carries it in the next brief under `## About this kit`, in full once per version and as a short reminder once a month after that, with the two lines that take the update. No routine runs the upgrade, and no routine runs `npx`.
...
- `soc-engagement-sweep` resolves each platform through 4b before it opens a tab, and `soc-publish-run` already resolves `channel.schedule` through this file. No routine names a vendor; the routes live in section 4b and nowhere else.
