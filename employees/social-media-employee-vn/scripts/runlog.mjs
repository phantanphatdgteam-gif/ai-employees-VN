...
/*
 * runlog.mjs
 * The only sanctioned way to append a run record to «SOC_ROOT»/runlog.jsonl.
 *
 ...
 * that carries a secret, post text, or personal data.
 *
 ...
 *     node scripts/runlog.mjs --routine soc-draft-queue --period 2026-03-05 \
 *          --start <ISO> --end <ISO> --status ok \
 *          --output "queue/2026-03-05-«platform».md (2 drafts)" \
 ...
 *                   folder, then the SOC_ROOT environment variable.
 *   --log <path>    Write to this file instead of «SOC_ROOT»/runlog.jsonl.
 ...
 *    "status":"ok",
 *    "outputs":["posts/posts.jsonl (+3 drafted)"],
 *    "blockers":[],
 *    "notes":"frameworks build-log, question, receipt"}
 *
 * EXIT CODES
 ...
 *   3  refused: the record carries a secret, post text, or personal data
...
const RETIRED_STATUSES = {
  ...
  "skipped-paused-run": "use skipped-paused",
  "published": "publishing is an outcome, not a run status. Use ok, and say what happened in outputs",
  "deferred": "deferred-no-scheduler is a ledger status on posts/posts.jsonl, never a run status. Use ok or partial",
};

// CONTRACT.md 1.3. Which period key shape each routine writes.
const CADENCE = {
  "soc-engagement-sweep": "day",
  "soc-calendar-standup": "day",
  "soc-publish-run": "day",
  "soc-material-sweep": "day",
  "soc-draft-queue": "day",
  "soc-performance-review": "week",
  "soc-intake-and-voice": "month",
};

// CONTRACT.md Appendix A. A stale id fails on its first line forever and the
// member never sees an error, so catch it here.
const STALE_IDS = {
  "social-calendar": "soc-calendar-standup",
  "calendar-standup": "soc-calendar-standup",
  "post-drafts": "soc-draft-queue",
  "soc-post-queue": "soc-draft-queue",
  "publish-run": "soc-publish-run",
  "soc-scheduler": "soc-publish-run",
  "listening-sweep": "soc-engagement-sweep",
  "soc-reply-queue": "soc-engagement-sweep",
  "content-sweep": "soc-material-sweep",
  "soc-idea-sweep": "soc-material-sweep",
  "weekly-social-review": "soc-performance-review",
  "soc-scorecard": "soc-performance-review",
  "social-intake": "soc-intake-and-voice",
  "soc-voice-build": "soc-intake-and-voice",
};
...
// Any URL. In this kit that is almost always a permalink, and a permalink is
// personal data: it names the post, and often the person who replied to it.
const ANY_URL = /\bhttps?:\/\/\S/i;
// A social handle. Matched only where it is not part of an address, which the
// email class above already catches.
const HANDLE = /(^|[\s(\[<"'])@[A-Za-z0-9._]{2,}/;
const PLACEHOLDER = /[«»]/; // guillemets, the kit's unresolved marker
const SALUTATION = /^(hi|hey|hello|dear|thanks for|great question)\b[^\n]{0,40}[,!?]/i;
...
function resolveRoot(opts) {
  ...
  // The script lives at «SOC_ROOT»/scripts/runlog.mjs, so its parent is the
...
function validateShape(record) {
  ...
  } else if (!CADENCE[routine] && !routine.startsWith("soc-")) {
    problems.push("routine \"" + routine + "\" is not one of the seven and does not carry the soc- prefix");
  }
...
function findRefusals(record) {
  ...
  for (const [field, value] of stringFields(record)) {
    ...
    if (ANY_URL.test(value)) refusals.push({ field, cls: "personal-data:url-or-permalink" });
    if (HANDLE.test(value)) refusals.push({ field, cls: "personal-data:handle" });
    if (value.includes("\n") || value.includes("\r")) refusals.push({ field, cls: "post-text:multiple-lines" });
    if (SUBJECT_LINE.test(value)) refusals.push({ field, cls: "post-text:subject-line" });
    if (SALUTATION.test(value.trim())) refusals.push({ field, cls: "post-text:salutation" });
    if (SIGN_OFF.test(value)) refusals.push({ field, cls: "post-text:sign-off" });
...
function good() {
  return {
    routine: "soc-draft-queue",
    period: "2026-03-05",
    start: "2026-03-05T09:15:11+07:00",
    end: "2026-03-05T09:41:40+07:00",
    status: "ok",
    outputs: ["posts/posts.jsonl (+3 drafted)", "material/material.jsonl (+3 drafted)"],
    blockers: [],
    notes: "1 draft dropped by copy-check, unsourced number; pillar cursor p-3",
  };
}
...
function selftest() {
  ...
  add("weekly key on a weekly routine", (r) => { r.routine = "soc-performance-review"; r.period = "2026-W10"; }, false, false);
  add("monthly key on a monthly routine", (r) => { r.routine = "soc-intake-and-voice"; r.period = "2026-03"; }, false, false);
  add("skipped-paused is a real status", (r) => { r.status = "skipped-paused"; r.outputs = []; r.notes = ""; }, false, false);
  ...
  add("stale routine id", (r) => { r.routine = "listening-sweep"; }, true, false);
  ...
  add("a permalink", (r) => { r.notes = "confirmed https://example.com/post/1"; }, false, true);
  add("a handle", (r) => { r.notes = "reply drafted for @somebody"; }, false, true);
  add("a post body", (r) => { r.notes = "Hi Sam, thanks for the question about the migration."; }, false, true);
  ...
  add("an unresolved placeholder", (r) => { r.period = "2026-03-05"; r.notes = "swept «source»"; }, false, true);
  ...
  try {
    ...
    fs.writeFileSync(tmpLog, "\uFEFF{\"routine\":\"soc-calendar-standup\",\"period\":\"2026-03-04\"}");
...
function main() {
  ...
  if (refusals.length) {
    ...
    die(3,
      "refused to write this record. A run record carries counts, slot ids, destination names, routes,\n" +
      "  paths, and blockers, never a secret, post text, or personal data (CONTRACT.md 4.2).\n" +
      "  The detail belongs in the queue file and the ledgers, inside «SOC_ROOT».\n" +
      "  " + lines.join("\n  ") +
