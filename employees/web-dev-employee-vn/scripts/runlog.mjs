...
/*
 * runlog.mjs
 * The only sanctioned way to append a run record to «WEB_ROOT»/runlog.jsonl.
 *
 ...
 *     node scripts/runlog.mjs --routine web-site-sweep --period 2026-03-04 \
 *          --start <ISO> --end <ISO> --status ok \
 *          --output "health/checks.jsonl (+14 checks, 1 fail)" \
 ...
 *                   folder, then the WEB_ROOT environment variable.
 *   --log <path>    Write to this file instead of «WEB_ROOT»/runlog.jsonl.
 ...
 *    "status":"ok",
 *    "outputs":["health/checks.jsonl (+14 checks, 1 fail)"],
 *    "blockers":[],
 *    "notes":"logs_cursor advanced on 4 surfaces; project cursor at 5 of 6"}
 *
 * EXIT CODES
...
//
// skipped-paused is the eighth and it exists because the pause switch is the
// first of this kit own four operator controls. A run stopped at Step 0.0 has
// to leave a record saying why, or a member who paused and forgot reads an
// unexplained hole in their ledgers instead of an explained gap.
...
const RETIRED_STATUSES = {
  ...
  "blocked-gate": "a failed gate is not a run status. Use ok or partial, and record gate-failed on the change line",
  "blocked-merge": "nothing in this kit merges anything. Use partial",
};
...
const CADENCE = {
  ...
  "web-standup": "day",
  "web-fix-runner": "day",
  "web-platform-guard": "week",
  "web-dependency-run": "week",
  "web-weekly-report": "week",
  "web-inventory-refresh": "month",
  "web-guardrail-review": "month",
};

// CONTRACT.md Appendix A. A stale id fails on its first line forever and the
// member never sees an error, so catch it here.
const STALE_IDS = {
  "site-sweep": "web-site-sweep",
  "health-sweep": "web-site-sweep",
  "web-health-sweep": "web-site-sweep",
  "standup": "web-standup",
  "triage-standup": "web-standup",
  "web-triage-standup": "web-standup",
  "fix-runner": "web-fix-runner",
  "web-change-runner": "web-fix-runner",
  "dependency-run": "web-dependency-run",
  "web-deps-run": "web-dependency-run",
  "platform-guard": "web-platform-guard",
  "web-provider-guard": "web-platform-guard",
  "weekly-report": "web-weekly-report",
  "web-change-report": "web-weekly-report",
  "inventory-refresh": "web-inventory-refresh",
  "web-intake-and-inventory": "web-inventory-refresh",
  "guardrail-review": "web-guardrail-review",
  "web-safe-fix-review": "web-guardrail-review",
  // A sibling Employee id landing here means --root resolved to the wrong kit.
  // Every Employee writes its own run log inside its own root.
  "gtm-board-standup": "web-standup, in this kit. A gtm- id here means --root resolved to the wrong kit",
  "ads-desk-standup": "web-standup, in this kit. An ads- id here means --root resolved to the wrong kit",
};
...
function resolveRoot(opts) {
  ...
  // The script lives at «WEB_ROOT»/scripts/runlog.mjs, so its parent is the
...
function validateShape(record) {
  ...
  } else if (!CADENCE[routine] && !routine.startsWith("web-")) {
    problems.push("routine \"" + routine + "\" is not one of the eight and does not carry the web- prefix");
  }
...
function good() {
  return {
    ...
    period: "2026-03-04",
    start: "2026-03-04T06:30:09+07:00",
    end: "2026-03-04T06:52:41+07:00",
    status: "ok",
    outputs: ["health/checks.jsonl (+14 checks, 1 fail)", "board/inbox.jsonl (+1 card)"],
    blockers: [],
    notes: "logs_cursor advanced on 4 surfaces; project cursor at 5 of 6",
  };
}
...
function selftest() {
  ...
  add("weekly key on a weekly routine", (r) => { r.routine = "web-weekly-report"; r.period = "2026-W10"; }, false, false);
  add("monthly key on a monthly routine", (r) => { r.routine = "web-guardrail-review"; r.period = "2026-03"; }, false, false);
  ...
  add("stale routine id", (r) => { r.routine = "fix-runner"; }, true, false);
  ...
  add("a sign off", (r) => { r.notes = "closed with Kind regards"; }, false, true);
  add("an unresolved placeholder", (r) => { r.period = "2026-03-04"; r.notes = "swept «source»"; }, false, true);
  ...
  try {
    ...
    fs.writeFileSync(tmpLog, "\uFEFF{\"routine\":\"web-standup\",\"period\":\"2026-03-03\"}");
