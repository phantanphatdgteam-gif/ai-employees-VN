...
- `cos-charter-and-fleet-audit` writes `evidence/sourced.md` exactly as `CONTRACT.md` shows it, and that template carries `Format: <the exact string that may appear in copy> | <ledger path it was read from> | <YYYY-MM-DD>` under `## Agent sourced`. The script read that line as an agent sourced entry without a real date, so every copy check in the install failed with a `proof-inventory` violation whatever the text said. A line that starts with `Format:` and still holds a `<...>` placeholder is now skipped as documentation. A real entry without a ledger path or a date is still rejected.
...
- The same fix as Chief of Staff 1.8.2. `scripts/` stays byte identical to the original kit.

## 1.9.0, 2026-09-24

The Vietnam localization pass: `chief-of-staff-vn` is a localized variant of `chief-of-staff` 1.8.1, built from the Vietnamese expert form for the Chief of Staff role (Phần A and sections B-1 to B-6). The form was filled by an AI and no clause has yet been confirmed by a Vietnamese practitioner.

- Six routines were localized from their own form section: `cos-charter-and-fleet-audit`, `cos-market-sweep`, `cos-metrics-review`, `cos-decision-brief`, `cos-decision-review` and `cos-fleet-reconcile`. `cos-fault-dossier` has no form section; under D15 its owner prose is Vietnamese and its parsed headings, metadata lines and correction line are unchanged. Routine instructions stay in English; every line the member reads is Vietnamese, and every heading, id and status another file parses stays in English.
- `CONTRACT.md` section 10, new, "Vietnam operating rules": the rules three or more routines share (platform terms, sale and holiday weeks, days off, no person's identity in a file, prices not seen, superlatives, the register tick, money figures never from a message, the member's language, no legal threshold in the kit), and the lines this variant adds to the charter, the decisions and the market files. `market/manual.md` joins the file map as a member only hand route, which the install creates once with a short Vietnamese header.
- `SCHEDULE.md`, `employee.json`, `CONTRACT.md`, `CAPABILITIES.md` and `README.md`: `cos-charter-and-fleet-audit` fires at 08:55 with its window from 08:40, before the lunch break and still its full budget plus twenty minutes before the Wednesday sweep.
- `CAPABILITIES.md`: a platform terms table checked 24/09/2026, the two public registers the audit reads, Vietnamese read only routes for `money.read` at `unknown`, marketplace facts for the metrics review, and the private chat delivery note for `brief.deliver`.
- `ROLE.md` section 5.1 gives the default working hours and lunch break, the shop's own sale days and the form of address; section 9 gains `sale or holiday week` and three `n/a` reasons.
- `INSTALL-PROMPT.md` asks for the form of address, working hours, days off including the company's own Tết option, the shop's sale days, the legal entity where the portal cannot be read, and the export folder, and tells the member to type the spending ceilings themselves.
...
- `examples/brief-latest.md` shows the Vietnamese lines under the English headings.
