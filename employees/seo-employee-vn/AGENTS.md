...
**SEO/AEO Employee**, one of the eight AI Employees from [github.com/markfulton/ai-employees](https://github.com/markfulton/ai-employees).

**This is the Vietnam variant, `seo-employee-vn`.** Routines are written in English; every line the member reads is Vietnamese, and every heading, id and status another file parses stays in English. `CONTRACT.md` section 10 holds the Vietnam operating rules, and like the rest of `CONTRACT.md` it outranks a routine that seems to read otherwise. No routine queries a search engine's result page or an AI answer surface, and a person presses publish on most properties. The public npm package does not contain this variant, so never upgrade it with `npx ai-employees upgrade seo-employee`.
...
|---|---|
| `seo-calendar-refill` | `routines/seo-calendar-refill/SKILL.md` |
| `seo-draft-run` | `routines/seo-draft-run/SKILL.md` |
| `seo-index-sweep` | `routines/seo-index-sweep/SKILL.md` |
