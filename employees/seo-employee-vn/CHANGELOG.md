...
The Vietnam localization pass: `seo-employee-vn` is the SEO/AEO Employee localized from `seo-employee` 1.9.0 for a business selling to buyers in Vietnam, from the SEO/AEO expert handoff form (`Khung-tri-thuc-ban-dia/03_SEO-AEO`, sections A and B-1 to B-8). The form was filled by an AI; no rule here has yet been confirmed by a person practising SEO in Vietnam.

- All eight routines localized, in English, with every line the member reads in Vietnamese. Every heading, key, status and path another file parses is unchanged, and Step 0, both guardrails, the run record and `## Corrections` are the original's.
- `CONTRACT.md` section 10, new, "Vietnam operating rules": the rules three or more routines apply, stated once without legal numbers; what the variant adds to the strategy files; the narrowed publish and indexing controls; the result sets and AI answers a person pastes; and what a person sends by hand.
- No routine queries a search engine's result page or an AI answer surface, and no routine reads a social, video, professional network or chat platform. A person views and pastes, into `research/result-sets/pasted/` and `tracking/answers/pasted/`, from ask sheets the calendar refill and the answer visibility routine write. `serp.read` is `unknown`, and IndexNow is used only for a property that sells abroad.
- A person presses publish on every property whose block does not say `publish_control: employee`, always on WordPress and for health supplements, cosmetics and medical devices. The publish run prints the label under an AI made hero and stops on a property with an open strange URL card.
- Health, superlative, price, place name and copying rules decide what may be written; alt text is a short Vietnamese phrase naming what the image shows; the hero comes from the member's own photos first.
- `strategy/properties.md` gains `publish_control`, `price_source`, `service_area`, a `Xưng hô:` line, `no new articles:`, `campaign:` and `season lock:` lines, and new threshold keys; `judgement_window` ships at 28 days. The intake writes the `Tết` season lock date from a public calendar page it read, never a computed one, and records the uses each industry paper names.
- `seo-answer-visibility` runs monthly (key `YYYY-MM`) on a set of twenty approved questions. The weekly rank review drafts a Vietnamese report block a person sends by hand.
- The monthly version check reads this variant's own folder, and the upgrade line uses the fork's bundled installer. The public npm package does not contain this variant.
...
- `routines/seo-standup/SKILL.md`, the step that writes the brief, gains the delivery line.
...
- `employee.json`: `standards/PUBLISH-STANDARD.md` ships with the kit, so it is now classified `kit` instead of `member`, and an upgrade can deliver a newer standard. Your copy is never overwritten: the new one lands beside it as `PUBLISH-STANDARD.md.new`. The `member` list also gains `PAUSED` and `schedule-commands.txt`.
...
- `seo-rank-review` resolves the performance and analytics reads through 4b before it opens a tab, and `seo-index-sweep` submits through 4b before it spends any of the console allowance. No routine names a vendor; the routes live in section 4b and nowhere else.
