...
- **Where orders are counted.** `strategy/utm-taxonomy.md` gains a member owned `## Order book` heading, defined in `CONTRACT.md` section 2.3. Intake fills it from your answers and carries it across verbatim. A website purchase event counts as orders only when more than 80 percent of the last thirty days' orders were paid on the website; otherwise orders come only from the dated counts you type into `scoreboard/manual.md`. No routine opens your order system. The paid guard, the scoreboard and the ICP refresh read this one definition instead of an undefined order file.
- `gtm-board-standup`: the brief template regains its per section rules and its fixed pointer line, which the 1.10.0 template had dropped while its own text still required them. The Vietnamese sample sits beside it, and a blocker line stays verbatim from its run record.
- `gtm-outreach-queue`: the bought list and consent check moves from the optional enrichment step, where most runs skipped it, into the do not touch set built before drafting. Vietnamese draft rules and Vietnamese queue header comments; the parsed lines are unchanged.
- `gtm-signal-sweep`: a hiring post older than 21 days survives only when its page shows an update in the last seven days, and that update date becomes the event date. A `strong` signal now also needs a contact path read on the page. The sweep records no phone number anywhere and no longer tells itself to file a card it has no right to write.
- `gtm-icp-refresh`: the Vietnam floors move into section 3.3 where the verdict reads them, and `months_of_signal` gains its approved value of two months.
- `gtm-launch-step-runner`: the five forms and three other cards per run were already the shipped ceilings, so the duplicate is removed. New: no new form is filled while five filled forms still wait for your submit.
- `gtm-scoreboard`: the rate floor is the member editable `rate_floor` again, not a fixed thirty.
- `gtm-intake-and-dashboard` and `CONTRACT.md` section 8.4: the monthly version check reads this variant's own folder, and the upgrade line uses the fork's installer, so the brief can never offer the English kit as an upgrade.
- `CAPABILITIES.md`: the `notify.push` table added in 1.9.0 is removed. The contract's own notification rule stands. `SCHEDULE.md`: the standup change note uses the section 9 format and the morning order prose matches the rows.
- `examples/`: one consistent fictional day. The English roofing queue file and the direct message file that did not match its own routine are replaced by one Vietnamese email queue entry.

## 1.10.0, 2026-09-24

Complete the first evidence bounded Vietnam pass across all eight GTM routines. This is a localized variant of `gtm-engineer` informed by `Phieu-chuyen-gia_08_GTM_da-dien.docx` and the matching extracted form. The original kit is untouched.

- `gtm-board-standup` now follows the reviewed source and preserves the original guards, ledgers, card rules, and parsed brief headings. The prior reminder sequence and automatic Zalo assumptions were not supported by a verified route and are removed from the routine and capability map. The previous draft is archived outside the kit for review.
- `gtm-intake-and-dashboard`, `gtm-icp-refresh`, `gtm-signal-sweep`, `gtm-outreach-queue`, `gtm-launch-step-runner`, `gtm-paid-and-tracking-guard`, and `gtm-scoreboard` now carry Vietnam evidence, owner language, and held action checks in their domain steps. Unrouted channels remain proposals, never automatic sends.
- The standup schedule references in `SCHEDULE.md`, `CONTRACT.md`, `README.md`, `CAPABILITIES.md`, and `INSTALL-PROMPT.md` agree with the schedule row and manifest. Its original budget and window start were restored after the unsupported reminder work was removed.
- See the external provenance report under `localization-reports/gtm-engineer-vn/` for adopted clauses, deferred clauses, and validator results. The fork's bundled installer lists this variant; the public npm installer does not.

## 1.9.0, 2026-09-24

The Vietnam variant of this kit, `gtm-engineer-vn`, with the morning brief localised first.

- `routines/gtm-board-standup/SKILL.md`, rewritten for a Vietnamese owner who reads the plan on a phone before the first customer call. The brief is Vietnamese: `Hôm nay`, `Chờ anh/chị`, `Đang kẹt`, dates as `dd/mm/yyyy`, money in đồng, the fixed second line `Em chưa bấm gì.`, and no footer. Hôm nay holds the owner's own presses, three a day by default and five at most, from `## Working days and hours` in `strategy/offer.md`. A waiting card is reminded in the brief on its first, second and fourth day and moves to Đang kẹt on the seventh, from a new `waiting_ages` field in the routine's state. A money or account incident sits first in Đang kẹt and is never compacted. Only a tick in a kit file makes a `sent` or a `done`; a message saying the drafts went out, or a request to record a send at a named time, is refused and named in the digest. Six invariants join the kit's four before the run record. Step 0, the two guardrails, the file map, the run record and every other shared section are the 1.8.0 text.
- `SCHEDULE.md`, the `gtm-board-standup` row: fire and window start later, budget 30 minutes, recorded in section 9.
- `CONTRACT.md` sections 2.7 and 8.4: the brief's headings and its fixed second line, in Vietnamese. Nothing else in the contract moved.
- `CAPABILITIES.md` section 6: the Vietnam route for `brief.deliver`, your own work account on Zalo, and a `notify.push` table with the same route. Every new row is `unknown` until watched.
- `examples/`: rewritten for a fictional Vietnamese business, the board header and the brief in Vietnamese.
- `employee.json`: slug `gtm-engineer-vn`, name `GTM Engineer VN`, version, and the standup row mirrored from `SCHEDULE.md`.
- The other seven routines are the 1.8.0 originals and still write English member text. They are localised in later versions.
...
- `routines/gtm-board-standup/SKILL.md`, the step that writes the brief, gains the delivery line.
...
- `gtm-intake-and-dashboard`, Step B3a, new: once a month it reads the published `VERSION` of this kit, and where there is a newer one it writes what you get, in at most five plain lines, to `state/kit-update.json`. `gtm-board-standup` carries it in the next brief under `## About this kit`, in full once per version and as a short reminder once a month after that, with the two lines that take the update. No routine runs the upgrade, and no routine runs `npx`.
...
- `gtm-paid-and-tracking-guard` resolves the conversion check through 4b before it opens a tab, and `gtm-outreach-queue` saves drafts through `mail.draft` where the member turned that on. No routine names a vendor; the routes live in section 4b and nowhere else.
...
- This kit also inherits five routine bodies hardened in live operation: the run record notes cap is named, a channel with no candidates gets no empty queue file, a copy check failure on a card field is repaired at the source, run records are never appended through a shell redirect, and the dashboard gains a Desk tab that carries the brief, the outreach drafts with copy controls, the latest signals, and the run history.
- The signal sweep now retires a source after three runs with zero contactable rows and spends the budget on sources that produce.
