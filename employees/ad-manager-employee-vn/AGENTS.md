...
**This is the Vietnam variant, `ad-manager-employee-vn`.** Routines are written in English; every line the member reads is Vietnamese, and every heading, id and status another file parses stays in English. `CONTRACT.md` section 10 holds the Vietnam operating rules, and it outranks a routine that seems to read otherwise. In this variant no routine creates, enables or spends anything in an ad account, whatever `RELEASES.md` says (section 10.3). The public npm package does not contain this variant, so never upgrade it with `npx ai-employees upgrade ad-manager-employee`.
