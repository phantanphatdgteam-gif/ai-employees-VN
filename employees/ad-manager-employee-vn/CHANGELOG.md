...
- All seven routines were localized from their own form section: `ads-account-intake`, `ads-account-read`, `ads-creative-studio`, `ads-build-desk`, `ads-change-list`, `ads-creative-retro` and `ads-desk-standup`. Routine instructions stay in English; every line the member reads is Vietnamese, and every heading, id and status another file parses stays in English. Step 0, the guardrails and every `## Corrections` section are unchanged.
- `CONTRACT.md` section 10, new, "Vietnam operating rules": the rules three or more routines share (products barred or held for a document, superlatives and promised results, platform terms, the order in the sales software as the deciding result, campaign types that count every order of the promoted products, sale and holiday periods, the learning phase, personal data, Vietnamese without diacritics, the member's language, no legal threshold in the kit), the lines this variant adds to the plan files, and section 10.3: the Employee's own authority over money is zero, so a `prepare` or `publish` row in `RELEASES.md` is read as `advise`. `exports/orders/` and `exports/ads/` join the file map as member only hand routes, and `employee.json` lists `exports/**` as member files.
- `plan/offer.md` gains `## Sale and holiday periods` and `## Creative pace`; `ads-account-intake` asks for them, for the sales software and its order statuses, the product documents, delivery provinces and communes, account administrators, working days and the form of address, one question at a time.
- `CAPABILITIES.md`: Vietnamese read only routes at `unknown`, a platform terms table and documentation hosts checked 24/09/2026, field limits read by a person, new section 4c with the dated channel facts a build sheet names, and a note that no route posts the brief into a chat app.
- `CONTRACT.md` section 8.4 and the intake's monthly check read this variant's own `VERSION`, and the upgrade line uses the fork's bundled installer, because the public npm package does not contain this variant.
- `SCHEDULE.md` and every fire time are unchanged: the two owner approved time changes contradict each other and are left for the owner (see the localization report).
- `ROLE.md`, `README.md`, `INSTALL-PROMPT.md`, `AGENTS.md` and `examples/build/campaign-storm-repair.md` carry the variant notes above.
...
- **Three operating modes**, `CONTRACT.md` section 7.0: `advise`, the shipped default where every channel is held; `prepare`, where the build desk creates an approved set as a paused campaign through a connected route; `publish`, where it activates too, inside the recorded budget. `RELEASES.md` selects the mode with one row that repeats the aggregate daily budget, and the row is yours alone. Every held clause about screens stays true in every mode: nothing is ever created through a browser.
- **`ads-build-desk` Step 6a, publication with receipts.** Six preconditions checked in order, the sequence from approved package to delivery observed, every id written to `build/publication-receipts.jsonl` the instant it returns, an uncertain response answered with a read and never a retry, replacement bookkept as `replaces`, activation applied to the campaign and its children, and configured status, platform status and delivery kept as three facts.
- **`recipes/META-ADS-RECIPES.md`, new.** The dependency chain intake verifies in order with one typed field per id, the budget semantics the platform does not explain, the publication sequence, the recoveries that were observed to work stated as observations, the rule that a connection proved in chat is not a connection proved on the schedule, the reading rules for account timezones and partial days, and the regression scenarios each of which was real.
- **`scripts/review.mjs`, new.** The member's review page on the loopback address: approve, needs revision, reject or withdraw, each a row in `creative/approvals.jsonl` bound to the exact revision of the manifest, the images and the money lines. Review, publication and delivery are read from three files and shown apart. Its self test covers the stale label that once showed a verified publication as queued.
- **The rejected set no longer blocks the studio.** `ads-creative-studio` 2.1: two open sets at most, a rejected or withdrawn set leaves the queue in the run that finds it, its note goes into `creative/feedback.md` and shapes the next set, a needs revision note is applied by the maintenance run, and every set opens with a `## Brief` naming the buyer situation, the visual hook, the message, the evidence, the format and the test. A template or a reference is an inspiration, never a proven performer.
- **Budgets are fields.** `plan/offer.md` gains `## Currency`, `## Campaign allocations` and `## Account timezone`; `## Daily cap` is the aggregate across every campaign; allocations are summed before any write; and `unresolved`, `0` and an authorised figure are three states. A blank answer is recorded as `unresolved`, never as zero, and no release publishes on either of the first two.
- **Measurement is five states.** `## Signal states` in `plan/measurement.md` and `signal_states{}` in the read routine's state: browser events, server events, purchase, deduplication, attribution, each verified on its own evidence and never inferred from a neighbour. An unverified purchase is a warning where the release row carries a measurement exception and a blocker otherwise, and today's partial spend is never written beside a completed day as if it were one.
- **The standup carries the state.** `operating-summary.md`, seven headings with a source and a date on every line and a resolved section so a fixed blocker stops being repeated; a receipt becomes a `live` row and closes the upload card, naming the receipt; a rejection parks the card; the brief shows approved, published and delivering as three labels.
- **Seven milestones, kept apart.** Installed, business context confirmed, connection verified in this session, schedules registered and read back, scheduled execution verified, production authorised, first publication verified. In intake state, in the install report, and refreshed monthly from the evidence. `## Platform identity` in `plan/account-map.md` is re-verified on every monthly pass and a resolved blocker is rewritten as resolved rather than left beside the newer fact.
- **`ads.account.write`** in `CAPABILITIES.md` section 4b and `employee.json`, held unless released, called by the build desk alone; `review.catalog` as the fourth kit capability; a second confidence mark on every connected route from the first scheduled read.
- The shared changes below, from the same install.
...
- `ads-account-read` resolves the account read and the conversion check through 4b before it opens a tab, and `ads-account-intake` does the same for the monthly structure read. No routine names a vendor; the routes live in section 4b and nowhere else.
