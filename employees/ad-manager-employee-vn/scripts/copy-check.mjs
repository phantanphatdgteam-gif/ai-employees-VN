...
/*
 * copy-check.mjs
 * The scripted judge for any text about to be written into a creative set, a
 * build sheet, a change list, a plan file, or a dashboard partial.
 *
 ...
 *                   folder, then the ADS_ROOT environment variable. Used to
 *                   find plan/voice.md and plan/proof-inventory.md.
 ...
 *      that exact string appears verbatim in plan/proof-inventory.md.
 ...
 *
 *   strategy skips the voice rules because plan/voice.md is the file that
 *   holds the banned lists, and a checker that fails its own vocabulary file
 *   is useless. dashboard skips hashtag and autolink because a hex colour and
 *   a markup attribute are not prose.
 *
 * EXIT CODES
 *   0  clean
...
/* ------------------------------------------------------------------ *
 * Rule 4. Voice. plan/voice.md overrides these lists completely when it
...
function loadVoice(root) {
  const file = path.join(root, "plan", "voice.md");
  ...
  return {
    ...
    source: "plan/voice.md",
  };
}

function loadProof(root) {
  const file = path.join(root, "plan", "proof-inventory.md");
  const text = readIfPresent(file);
  ...
  return { allowed: allowed.join("\n"), structural, source: "plan/proof-inventory.md" };
}
...
function check(text, dest, ctx) {
  ...
  for (const [kind, pattern] of METRIC_PATTERNS) {
    ...
    while ((m = re.exec(metricView)) !== null) {
      ...
      at(m.index, m[0].length, "metric",
        ...
        (ctx.proof.source === "missing" ? "plan/proof-inventory.md, which is missing" : "plan/proof-inventory.md"));
    }
  }
  ...
  for (const bad of ctx.proof.structural) {
    v.push({
      ...
      detail: "plan/proof-inventory.md, under ## Agent sourced, has an entry that is not " +
 ...
 * the member's own plan files.
 * ------------------------------------------------------------------ */

function selftest() {
  const ctx = { voice: { ...FALLBACK_VOICE, source: "fallback" }, proof: { allowed: "", structural: [], source: "missing" } };
  const withProof = { voice: ctx.voice, proof: { allowed: "42% reply rate", structural: [], source: "plan/proof-inventory.md" } };
  ...
  const cases = [
    ...
    ["a path in backticks is fine", "The ledger is `metrics/daily.jsonl` and it appends.", "strategy", ctx, []],
    ["a bare kit path is fine", "Write it to metrics/daily.jsonl every run.", "plain", ctx, []],
    ["a filename is not a link", "Open changes/change-list-2026-W10.md and tick the boxes.", "plain", ctx, []],
