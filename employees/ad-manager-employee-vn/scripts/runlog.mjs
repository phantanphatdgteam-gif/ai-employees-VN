...
/*
 * runlog.mjs
 * The only sanctioned way to append a run record to «ADS_ROOT»/runlog.jsonl.
 *
 ...
 *     node scripts/runlog.mjs --routine ads-account-read --period 2026-03-04 \
 *          --start <ISO> --end <ISO> --status ok \
 *          --output "metrics/daily.jsonl (+41 rows, 1 reporting day)" \
 ...
 *   --log <path>    Write to this file instead of «ADS_ROOT»/runlog.jsonl.
 ...
 *    "status":"ok",
 *    "outputs":["metrics/daily.jsonl (+41 rows, 1 reporting day)"],
 *    "blockers":[],
 *    "notes":"conversion event fired in the read window"}
 *
 * EXIT CODES
...
//
// skipped-paused is the eighth and it exists because the pause switch is one of
// this kit s four operator controls. A run stopped at Step 0.0 has to leave a
// record that says why, or a member who paused and forgot reads an unexplained
// hole in their ledgers instead of an explained gap.
...
const CADENCE = {
  "ads-account-read": "day",
  "ads-desk-standup": "day",
  "ads-creative-studio": "day",
  "ads-build-desk": "day",
  "ads-change-list": "week",
  "ads-account-intake": "month",
  "ads-creative-retro": "month",
};

// CONTRACT.md Appendix A. A stale id fails on its first line forever and the
// member never sees an error, so catch it here.
const STALE_IDS = {
  "account-read": "ads-account-read",
  "ads-account-reader": "ads-account-read",
  "ads-metrics-read": "ads-account-read",
  "desk-standup": "ads-desk-standup",
  "ads-standup": "ads-desk-standup",
  "creative-studio": "ads-creative-studio",
  "ads-creative": "ads-creative-studio",
  "build-desk": "ads-build-desk",
  "ads-build-sheet": "ads-build-desk",
  "change-list": "ads-change-list",
  "ads-weekly-review": "ads-change-list",
  "account-intake": "ads-account-intake",
  "ads-intake-and-dashboard": "ads-account-intake",
  "creative-retro": "ads-creative-retro",
  "ads-creative-retrospective": "ads-creative-retro",
  // The sibling kit. Its routines write their own run log in their own root,
  // and a gtm- id landing here means the wrong root was resolved.
  "gtm-paid-and-tracking-guard": "ads-account-read, in this kit. A gtm- id here means --root resolved to the wrong kit",
};
...
function resolveRoot(opts) {
  ...
  if (process.env.ADS_ROOT) return path.resolve(process.env.ADS_ROOT);
  // The script lives at «ADS_ROOT»/scripts/runlog.mjs, so its parent is the
...
function validateShape(record) {
  ...
  } else if (!CADENCE[routine] && !routine.startsWith("ads-")) {
    problems.push("routine \"" + routine + "\" is not one of the seven and does not carry the ads- prefix");
  }
...
function good() {
  return {
    ...
    period: "2026-03-04",
    start: "2026-03-04T06:45:11+07:00",
    end: "2026-03-04T07:09:40+07:00",
    status: "ok",
    outputs: ["metrics/daily.jsonl (+41 rows, 1 reporting day)", "board/inbox.jsonl (+2 cards)"],
    blockers: [],
    notes: "conversion event fired in the read window; 4 levels read on 2 accounts",
  };
}
...
function selftest() {
  ...
  add("weekly key on a weekly routine", (r) => { r.routine = "ads-change-list"; r.period = "2026-W10"; }, false, false);
  add("monthly key on a monthly routine", (r) => { r.routine = "ads-creative-retro"; r.period = "2026-03"; }, false, false);
  ...
  add("stale routine id", (r) => { r.routine = "change-list"; }, true, false);
  ...
  try {
    ...
    fs.writeFileSync(tmpLog, "\uFEFF{\"routine\":\"ads-desk-standup\",\"period\":\"2026-03-03\"}");
